...
data <- data %>%
        mutate(suspect.race = factor(suspect.race, levels = c("white", "black", "asian", "hispanic", "native.american", "other", "unknown")),
...
glm_data <- glm_data %>% 
  mutate(predicted = predict(glm.found))
